public class Exercise08_07{
  public static void main(String[] args){
    ...
    int p1 = 0;
    int p2 = 1;
    double shortestDistance = distance(points[p1][0], points[p1][1], points[p2][0], points[p2][1], points[p1][2], points[p2][2]);
    for(int i = 0; i < points.length; i++){
      for(int j = i+1; j < points.length; j++){ 
        for(int x = i+1; x < points.length; x++){
          double distance = distance(points[i][0], points[i][1], points[j][0], points[j][1], points[i][2], points[j][2]);
